function (moduleHelpers) {
  ...
  var audioPlayed = [];
  import_moduleHelpers.context.moduleContexts.stereo_sound.state = {
    audioPlayed
  };
  var audioplayer;
  var isPlaying;
  ...
  function make_stereo_sound(left_wave, right_wave, duration) {
    return pair(pair(t => t >= duration ? 0 : left_wave(t), t => t >= duration ? 0 : right_wave(t)), duration);
  }
  ...
  function simultaneously(list_of_sounds) {
    ...
    const unnormed = accumulate(stereo_simul_two, silence_sound(0), list_of_sounds);
    const sounds_length = length(list_of_sounds);
    const normalised_left = t => head(head(unnormed))(t) / sounds_length;
    const normalised_right = t => tail(head(unnormed))(t) / sounds_length;

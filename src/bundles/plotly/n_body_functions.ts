...
export var G_const = 1; // Gravitational Constant
...
export class Body {
    ...
    rungeKuttaCalculation(dt: number, v: number[], dv: number[]) {
        var a = [dt / 2, dt / 2, dt, 0];
        var b = [dt / 6, dt / 3, dt / 3, dt / 6];
        var v0: number[] = [];
        var vt: number[] = [];
                ...
                for (var i = 0; i < bodies.length; i++) {
                    for (var childNo = 0; childNo < 4; childNo++) {
                        if (this.children[childNo] == null) {
                            this.children[childNo] =
                                QuadTreeNode.getEmptyNode();
                            this.children[childNo].bounding_box =
                                this.getBoundingBox(childNo, this.bounding_box);
                        }

                        if (
                            this.children[childNo].bounding_box.contains_point(
                                bodies[i].pos
                            )
                        ) {
                            this.children[childNo].insert_body(
                                bodies[i],
                                depth + 1
                            );
                        }
            ...
            if (s / d < theta && this.body.length > 0) {
                ...
                for (let i = 0; i < 4; i++) {
                    if (this.children[i] != null) {
                        const forceActed =
                            this.children[i].netForceOnBody(body);

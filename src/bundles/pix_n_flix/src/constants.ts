// Default values of video
export const DEFAULT_WIDTH: number = 400;
export const DEFAULT_HEIGHT: number = 300;
export const DEFAULT_FPS: number = 10;
export const DEFAULT_VOLUME: number = 0.5;
export const DEFAULT_LOOP: number = Infinity;

// Maximum values allowed for video
export const MAX_HEIGHT: number = 1024;
export const MIN_HEIGHT: number = 1;
export const MAX_WIDTH: number = 1024;
export const MIN_WIDTH: number = 1;
export const MAX_FPS: number = 60;
export const MIN_FPS: number = 1;

...
class VectorCalculus extends React.Component<Props, State> {
  ...
  public render() {
    const { context: { moduleContexts: { vector_calculus: { state: { drawnVectors } } } } } = this.props.debuggerContext;
      ...
      <div>
        ...
        </Modal>
        {
          drawnVectors.map((drawnVector: any, id:number) => {
            ...
            return (
              <>
                <div onClick={() => this.handleOpen(drawnVector)}>Click here to open Modal</div>
                <div
                  id={divId}
                  ref={() => {
...
export default {
  toSpawn(context: DebuggerContext) {
    const drawnVectors = context.context?.moduleContexts?.vector_calculus.state.drawnVectors;
    return drawnVectors.length > 0;
  },
  body: (debuggerContext: any) => <VectorCalculus debuggerContext={debuggerContext} />,

...
import type {
  ...
  Numbers2,
  Vector
} from '@sourceacademy/bundle-csg/jscad/types';

/* [Main] */
/* NOTE
  Hand-written typings for the untyped internals of @jscad/regl-renderer. These
  used to sit beside the geometry types in the bundle; they moved here with the
  renderer when csg migrated to Conductor, since regl only ever runs on the
  browser's main thread. The geometry-level types they build on are still
  imported from the bundle, which is the one side that defines what a Solid is.
*/

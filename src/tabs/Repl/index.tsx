  ...
  public render() {
    ...
    for (let i = 0; i < outputStringCount; i++) {
      ...
      if (str.outputMethod === 'richtext') {
        if (str.color === '') {
          outputDivs.push(
            <div style={FONT_MESSAGE} dangerouslySetInnerHTML={{ __html: str.content }} />
          );
        } else {
          outputDivs.push(
            <div
...
export default {
  ...
  body(context: DebuggerContext) {
    return (
      <ProgrammableReplGUI programmableReplInstance={context.context.moduleContexts.repl.state} />
    );
  },

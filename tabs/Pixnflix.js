(function (React) {
    ...
    */
    ...
    var DEFAULT_WIDTH=400;var DEFAULT_HEIGHT=300;var DEFAULT_FPS=10;var DEFAULT_VOLUME=0.5;var MAX_HEIGHT=1024;var MIN_HEIGHT=1;var MAX_WIDTH=1024;var MIN_WIDTH=1;var MAX_FPS=60;var MIN_FPS=1;
    ...
    var VideoMode;(function(VideoMode){VideoMode[VideoMode["Video"]=0]="Video";VideoMode[VideoMode["Still"]=1]="Still";VideoMode[VideoMode["Accepting"]=2]="Accepting";VideoMode[VideoMode["Image"]=3]="Image";})(VideoMode||(VideoMode={}));var PixNFlix=function(_super){__extends(PixNFlix,_super);function PixNFlix(props){var _this=_super.call(this,props)||this;_this.$video=null;_this.$image=null;_this.$canvas=null;_this.setupVideoService=function(){if(_this.$video&&_this.$canvas&&_this.isPixNFlix()){var debuggerContext=_this.props.debuggerContext;_this.pixNFlix=debuggerContext.result.value;var _a=_this.pixNFlix.init(_this.$image,_this.$video,_this.$canvas,_this.printError,{onClickStill:_this.onClickStill}),HEIGHT=_a.HEIGHT,WIDTH=_a.WIDTH,FPS=_a.FPS,VOLUME=_a.VOLUME,inputFeed=_a.inputFeed;var mode=VideoMode.Video;if(inputFeed===InputFeed.Local){mode=VideoMode.Accepting;}else if(inputFeed===InputFeed.ImageURL){mode=VideoMode.Image;}_this.setState({height:HEIGHT,width:WIDTH,FPS:FPS,volume:VOLUME,hasAudio:inputFeed===InputFeed.VideoURL,mode:mode});}};_this.closeVideo=function(){if(_this.isPixNFlix()){_this.pixNFlix.deinit();}};_this.handleStartVideo=function(){if(_this.isPixNFlix()){_this.pixNFlix.startVideo();}};_this.handleStopVideo=function(){if(_this.isPixNFlix()){_this.pixNFlix.stopVideo();}};_this.onClickStill=function(){var mode=_this.state.mode;if(mode===VideoMode.Still){_this.handleStopVideo();}else if(mode===VideoMode.Video){_this.setState(function(){return {mode:VideoMode.Still}},_this.handleStopVideo);}};_this.onClickVideo=function(){var mode=_this.state.mode;if(mode===VideoMode.Still){_this.setState(function(){return {mode:VideoMode.Video}},_this.handleStartVideo);}};_this.handleWidthChange=function(width){var height=_this.state.height;_this.handleUpdateDimensions(width,height);};_this.handleHeightChange=function(height){var width=_this.state.width;_this.handleUpdateDimensions(width,height);};_this.handleFPSChange=function(fps){if(fps>=MIN_FPS&&fps<=MAX_FPS){_this.setState({FPS:fps});if(_this.isPixNFlix()){_this.pixNFlix.updateFPS(fps);}}};_this.handleUpdateDimensions=function(w,h){if(w>=MIN_WIDTH&&w<=MAX_WIDTH&&h>=MIN_HEIGHT&&h<=MAX_HEIGHT){_this.setState({width:w,height:h});if(_this.isPixNFlix()){_this.pixNFlix.updateDimensions(w,h);}}};_this.loadFileToVideo=function(file){var mode=_this.state.mode;if(file.type.match("video.*")){if(_this.$video&&mode===VideoMode.Accepting){_this.$video.src=URL.createObjectURL(file);_this.setState({hasAudio:true,mode:VideoMode.Video});_this.handleStartVideo();}}else if(file.type.match("image.*")){if(_this.$image&&mode===VideoMode.Accepting){_this.$image.src=URL.createObjectURL(file);_this.setState({mode:VideoMode.Image});}}};_this.handleDrop=function(e){e.preventDefault();_this.loadFileToVideo(e.dataTransfer.files[0]);};_this.handleDragOver=function(e){e.preventDefault();};_this.handleFileUpload=function(e){e.preventDefault();if(e.target&&e.target.files){_this.loadFileToVideo(e.target.files[0]);}};_this.handleVolumeChange=function(e){e.preventDefault();var volume=parseFloat(e.target.value);_this.setState({volume:volume});_this.pixNFlix.updateVolume(volume);};_this.printError=function(){};_this.state={width:DEFAULT_WIDTH,height:DEFAULT_HEIGHT,FPS:DEFAULT_FPS,volume:DEFAULT_VOLUME,hasAudio:false,mode:VideoMode.Video};var debuggerContext=_this.props.debuggerContext;_this.pixNFlix=debuggerContext.result.value;return _this}PixNFlix.prototype.componentDidMount=function(){if(this.isPixNFlix()){this.setupVideoService();window.addEventListener("beforeunload",this.pixNFlix.deinit);}};PixNFlix.prototype.componentWillUnmount=function(){if(this.isPixNFlix()){this.closeVideo();window.removeEventListener("beforeunload",this.pixNFlix.deinit);}};PixNFlix.prototype.isPixNFlix=function(){return this.pixNFlix&&this.pixNFlix.toReplString&&this.pixNFlix.toReplString()==="[Pix N Flix]"};PixNFlix.prototype.render=function(){var _this=this;var _a=this.state,mode=_a.mode,width=_a.width,height=_a.height,FPS=_a.FPS,volume=_a.volume,hasAudio=_a.hasAudio;var displayOptions=mode===VideoMode.Still||mode===VideoMode.Video;var videoIsActive=mode===VideoMode.Video;var isAccepting=mode===VideoMode.Accepting;return React__default['default'].createElement("div",{className:"sa-video",onDragOver:this.handleDragOver,onDrop:this.handleDrop},React__default['default'].createElement("div",{className:"sa-video-header"},React__default['default'].createElement("div",{className:"sa-video-header-element",style:{display:displayOptions?"inherit":"none"}},React__default['default'].createElement(ButtonGroup,null,React__default['default'].createElement(Button,{className:"sa-live-video-button",icon:VIDEO,active:videoIsActive,onClick:this.onClickVideo,text:"Play Video"}),React__default['default'].createElement(Button,{className:"sa-still-image-button",icon:CAMERA,active:!videoIsActive,onClick:this.onClickStill,text:"Pause Video"}))),React__default['default'].createElement(Divider,null),React__default['default'].createElement("div",{className:"sa-video-header-element",style:{display:displayOptions?"inherit":"none"}},React__default['default'].createElement("div",{className:"sa-video-header-numeric-input"},React__default['default'].createElement(NumericInput,{disabled:true,leftIcon:HORIZONTAL_DISTRIBUTION,style:{width:70},value:width,onValueChange:this.handleWidthChange,minorStepSize:1,stepSize:10,majorStepSize:100,max:MAX_WIDTH,min:MIN_WIDTH})),React__default['default'].createElement("div",{className:"sa-video-header-numeric-input"},React__default['default'].createElement(NumericInput,{disabled:true,leftIcon:VERTICAL_DISTRIBUTION,style:{width:70},value:height,onValueChange:this.handleHeightChange,minorStepSize:1,stepSize:10,majorStepSize:100,max:MAX_HEIGHT,min:MIN_HEIGHT})),React__default['default'].createElement("div",{className:"sa-video-header-numeric-input"},React__default['default'].createElement(NumericInput,{leftIcon:STOPWATCH,style:{width:60},value:FPS,onValueChange:this.handleFPSChange,minorStepSize:null,stepSize:1,majorStepSize:null,max:MAX_FPS,min:MIN_FPS})))),React__default['default'].createElement("div",{className:"sa-video-element"},React__default['default'].createElement("img",{ref:function(r){_this.$image=r;},width:DEFAULT_WIDTH,height:DEFAULT_HEIGHT,style:{display:"none"}}),React__default['default'].createElement("video",{ref:function(r){_this.$video=r;},autoPlay:true,width:DEFAULT_WIDTH,height:DEFAULT_HEIGHT,style:{display:"none"}}),React__default['default'].createElement("canvas",{ref:function(r){_this.$canvas=r;},width:DEFAULT_WIDTH,height:DEFAULT_HEIGHT,style:{display:!isAccepting?"initial":"none"}}),React__default['default'].createElement("br",null),React__default['default'].createElement("div",{style:{display:isAccepting?"inherit":"none"}},React__default['default'].createElement("div",{style:{fontSize:40}},"Drag file here"),React__default['default'].createElement("br",null),React__default['default'].createElement("input",{type:"file",onChange:this.handleFileUpload})),React__default['default'].createElement("br",null),React__default['default'].createElement("div",{style:{display:hasAudio&&!isAccepting?"inherit":"none"}},"Volume:",React__default['default'].createElement("input",{type:"range",onChange:this.handleVolumeChange,min:0,max:1,value:volume,step:0.01})),React__default['default'].createElement("p",{style:{display:displayOptions?"inherit":"none",fontFamily:"arial"}},"Note: Is video lagging? Switch to 'still image' or adjust FPS rate!")))};return PixNFlix}(React__default['default'].Component);var index = {toSpawn:function(){return true},body:function(debuggerContext){return React__default['default'].createElement(PixNFlix,{debuggerContext:debuggerContext})},label:"PixNFlix Live Feed",iconName:"mobile-video"};

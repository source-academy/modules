(function (React, ReactDOM) {
    ...
    */
    ...
    function HollusionCanvas(_a){var rune=_a.rune;var canvasRef=React__default['default'].useRef(null);var renderFuncRef=React__default['default'].useRef();var animId=React__default['default'].useRef(null);var animCallback=function animCallback(timeInMs){renderFuncRef.current(timeInMs);animId.current=requestAnimationFrame(animCallback);};React__default['default'].useEffect(function(){if(canvasRef.current){renderFuncRef.current=rune.draw(canvasRef.current);animCallback(0);return function(){if(animId.current){cancelAnimationFrame(animId.current);}}}return undefined},[]);return React__default['default'].createElement(WebGLCanvas,{ref:canvasRef})}

    var index = {toSpawn:function(context){var _a;var moduleContext=(_a=context.context)===null||_a===void 0?void 0:_a.moduleContexts.get("rune");if(!moduleContext){return false}var moduleState=moduleContext.state;if(!moduleState){return false}return moduleState.drawnRunes.length>0},body:function(context){var _a;var moduleContext=(_a=context.context)===null||_a===void 0?void 0:_a.moduleContexts.get("rune");var moduleState=moduleContext.state;var runeCanvases=moduleState.drawnRunes.map(function(rune,i){var elemKey=i.toString();if(glAnimation.isAnimation(rune)){return React__default['default'].createElement(AnimationCanvas,{animation:rune,key:elemKey})}var drawnRune=rune;if(drawnRune.isHollusion){return React__default['default'].createElement(HollusionCanvas,{rune:drawnRune,key:elemKey})}return React__default['default'].createElement(WebGLCanvas,{ref:function(r){if(r){drawnRune.draw(r);}},key:elemKey})});return React__default['default'].createElement(MultiItemDisplay,{elements:runeCanvases})},label:"Runes Tab",iconName:"group-objects"};

...
export const processOutputFiles = async (outputFiles, outDir, startTime) => Promise.all(outputFiles.map(async ({ path, text }) => {
...
export const buildModules = async (buildOpts, { bundles, tabs }) => {
    const startPromises = [];
    if (bundles.length > 0) {

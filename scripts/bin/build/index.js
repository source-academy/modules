...
const buildAllCommand = async (buildOpts) => {
...
export default new Command('build')
    ...
    .action(async (subcommand, rawOpts) => {
    ...
    if (rawOpts.modules.length === 0) {
        ...
        const undefineds = rawOpts.modules.reduce((res, module) => (!rawOpts.modules.includes(module) ? [...res, module] : res), []);
    ...
    if (!subcommand)
        console.log(chalk.redBright(`Unknown build command: "${subcommand}"`));
    else
        await subfunc(buildOpts);
});

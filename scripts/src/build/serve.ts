...
const incrementalBuilder = async ({ verbose, ...opts }: ServeCommandOpts) => {
    ...
    plugins: [{
      ...
      setup(pluginBuild) {
        ...
        pluginBuild.onEnd(async ({ outputFiles }) => {
          const buildDocs = () => {
            if (!opts.docs) return Promise.resolve(false as false);
...
const createServeCommand = (name: string, action: (opts: ServeCommandOpts) => Promise<void>) => new Command(name)
...
export const watchCommand = createServeCommand('watch', async (opts) => {
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    const endRes = (code: number, msg?: string) => {

...
export function findSeverity<T extends object>(
  results: T[],
...
export type AwaitedReturn<T> = T extends (...args: any) => Promise<infer U>
  ? U
  : never;
...
type LogType = Partial<
  Record<AssetType, OperationResult[]> & {
    html: Awaited<ReturnType<typeof buildHtml>>;
  }
>;
...
function processResults(results: LogType, verbose: boolean) {
  ...
  ): result is Exclude<OperationResult, SuccessResult> =>

...
(function () {
  ...
  const obj_types = [image_type, text_type, rect_type, container_type];
  ...
  function get_obj(scene, id) {
    ...
    const rect = create_rect(0, 0, 0, 0)(scene, id);
    if (rect) return rect;
  ...
  function create_text(x, y, text, style) {
    ...
    return function (scene, id = -1) {
      if (id === -1) {
        ...
        return create_text.text_map[id];
      }
    };
  }
  ...
  function create_rect(x, y, width, height, fill = 0, alpha = 1) {
    // Set up local mapping
    if (typeof create_rect.rect_map === "undefined") {
      create_rect.rect_map = {};
    }
    ...
    return function (scene, id = -1) {
      if (id === -1) {
        ...
        // Set text settings
        rect.setDataEnabled();
        rect.data.set(type_key, rect_type);
        rect.data.set(id_key, new_id);

        // Insert into the map
        create_rect.rect_map[new_id] = rect;
        return new_id;
      } else {
        return create_rect.rect_map[id];
      }
    };
  }

...
(_params) => {
  ...
  const preload_spritesheet_map = _params.preloadSpritesheetMap;
  ...
  /**
   * Checks whether the given key is a spritesheet key.
   *
   * @param {string} key key to check
   * @returns {boolean}
   */
  function is_spritesheet_key(key) {
    const spritesheet = preload_spritesheet_map.get(key);
    return spritesheet !== undefined;
  }
  ...
  function load_spritesheet(key, url, spritesheet_config) {
    preload_spritesheet_map.set(key, { path: url, config: spritesheet_config });
  }

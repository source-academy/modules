(_params) => {
  ...
  function create_obj(creator_fn, type) {
    const obj = creator_fn();
    obj.setDataEnabled();
  ...
  function create_container(x, y) {
    const cont_creator = () => new Phaser.GameObjects.Container(scene, x, y);
    return create_obj(cont_creator, container_type);
  }
  ...
  const final_functions = functions.map(fn => {
    (!Phaser || !scene) ? () => {} : fn
  });
